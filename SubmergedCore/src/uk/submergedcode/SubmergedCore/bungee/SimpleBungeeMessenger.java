 ...
 */
package uk.submergedcode.SubmergedCore.bungee;
...
public class SimpleBungeeMessenger implements BungeeMessenger {
    ...
    public SimpleBungeeMessenger() {
        ...
        Bukkit.getPluginManager().registerEvents(new BungeePlayerListener(this), SubmergedCore.getInstance());
    }
    ...
    public boolean sendRawMessage(byte[] message, boolean shouldQueue) {
        if (Bukkit.getOnlinePlayers().length > 0) {
            ...
            p.sendPluginMessage(SubmergedCore.getInstance(), "BungeeCord", message);
            return true;
        } else if (shouldQueue) {
            queue.add(message);
        }
    ...
    public void sendRawPlayerMessage(Player player, byte[] message) {
        if (player.isOnline()) {
            player.sendPluginMessage(SubmergedCore.getInstance(), "BungeeCord", message);
        }
    }
