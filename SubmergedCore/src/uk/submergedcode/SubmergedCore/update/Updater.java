...
public abstract class Updater {
    ...
    public Updater(Module module, String string) {
        ...
        m_downloadFolder = new File(SubmergedCore.getModuleLoader().getModuleDirs().get(0) + File.separator + ".update" + File.separator);
        m_downloadFolder.mkdirs();

        m_backupFolder = new File(SubmergedCore.getModuleLoader().getModuleDirs().get(0) + File.separator + ".backup" + File.separator);
    ...
    public void update() throws Exception {
        if (checkUpdate()) {
            if (SubmergedCore.getConfigurationManager().isAutoDownloadEnabled()) {
                downloadUpdate();
            }
            if (SubmergedCore.getConfigurationManager().isAutoInstallEnabled()) {
                applyUpdate();
            }
        }
    }
